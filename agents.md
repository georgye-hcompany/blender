...
- Multiple `alt-id` descriptions are strongly preferred whenever they improve localization quality. Store multiple IDs in the same attribute separated by semicolons `;` rather than settling for a single label.
- Any `*-id` attribute may contain multiple IDs on the same element; separate them with semicolons `;`.
...
- If useful, an `alt-id` may contain multiple semicolon-separated descriptions combining those dimensions.
...
- If multiple descriptions help, separate them with semicolons `;`
